...
def smooth2(a, nx, ny):
    ...
    #b=np.squeeze(a.cpu().numpy())
    b=a.cpu().numpy()
    r=np.zeros_like(a.cpu().numpy())

    for i in range(a.shape[0]):
    
      #r[i,0,:,:] = signal.convolve2d(b[i,:,:], kernel, mode = 'same')
      r[i,0,:,:]=cv.GaussianBlur(b[i,0,:,:], (nx, ny), 5)
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, classes, cond_scale = 6., rescaled_phi = 0.7, clip_x_start = False):
        if self.conditional==True:
          model_output = self.denoise_fn(classes,x, t)
        else:
    ...
    def super_resolution(self, x_in, continous=False):
        return self.ddim_sample(torch.zeros_like(x_in['SR']),x_in['HR'], x_in['HR'].shape)

    @torch.no_grad()
   
    def ddim_sample(self, classes, data, shape, cond_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            ret_img = torch.cat([ret_img, x_start], dim=1)                      
            time_last=time
            timestep+=1        
        return img, ret_img
    ...
    def p_losses(self, x_start, c,noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(
                torch.zeros_like(x_in['SR']), x_noisy, t)\
        
        loss = F.mse_loss(noise, x_recon, reduction = 'none')
