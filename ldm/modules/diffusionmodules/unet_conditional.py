...
class SelfAttention(nn.Module):
    def __init__(self, in_channel, n_head=8, norm_groups=32):
        super().__init__()
...
class UNet(nn.Module):
    def __init__(
        ...
        now_res = image_size
        pres_channel=5 
        pres=[torch.nn.ZeroPad2d(padding=(0,0,0,105))]
        pres.append(nn.Conv2d(pres_channel, 2*pres_channel,
                           kernel_size=3, stride=[2,1],padding=1))   
        pres.append(nn.Conv2d(2*pres_channel, 4*pres_channel,
                           kernel_size=3, stride=[2,1],padding=1))        
        pres.append(nn.Conv2d(4*pres_channel, 16*pres_channel,
                           kernel_size=3, stride=[2,1],padding=1)) 
        pres.append(nn.Conv2d(16*pres_channel, 64*pres_channel,
                           kernel_size=3, stride=[2,1],padding=1)) 
    ...
    def forward(self, g,m, time):
        t = self.time_mlp(time) if exists(self.time_mlp) else None

        for layer in self.pres:
        ...
        for layer in self.ups:
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                if ii in range (4,7):
                  x=torch.cat([x, F.pad(feats.pop(),(0,1,0,1), 'constant', 0)], dim=1)
                elif ii in range (8,11):
                  x=torch.cat([x, F.pad(feats.pop(),(1,1,1,1), 'constant', 0)], dim=1)
                elif ii in range (12,15):
                  x=torch.cat([x, F.pad(feats.pop(),(2,3,2,3), 'constant', 0)], dim=1)
                elif ii in range (16,19):
                  x=torch.cat([x, F.pad(feats.pop(),(5,5,5,5), 'constant', 0)], dim=1)             
                else:
